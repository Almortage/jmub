...
async def hi(event):
    await edit_or_reply(
        event,
        "**[ . ᯏAVATAR - ᥴ𝗆𝖽 ᭡ .](t.me/source_av)**\n✦┅━╍━╍╍━━╍━━╍━┅✦\n\n• اوامر التحكم تتيح لمستخدم اخر ان يستخدم اوامر تنصيبك وهو غير منصب جمثون افاتار حيث سيصبح متحكم في اوامر تنصيبك يرجى الانتباه من انه سيحصل على صلاحيات الاوامر  وقد يسبب خطورة لك اذا كنت لا تثق في المستخدم الذي اضفته. \n\n\n`.التحكم` تفعيل/تعطيل\n• تستخدم هذه الميزة لتفعيل/لتعطيل خاصية التحكم بأوامر تنصيبك للمستخدمين الذين أضفتهم في قائمة المتحكمين\n\n`.المتحكمين`\n• لعرض المستخدمين الذين اضفتهم الى قائمة المستخدمين المتحكمين\n\n`.اضف متحكم`\n• بالرد على المستخدم لأضافته متحكم في تنصيب جمثون افاتار الخاص بك\n\n`.ازالة متحكم`\n• بالرد على المستخدم لازالته من قائمة المتحكمين",
        link_preview=False,
    )
...
async def _(event):
    ...
    if len(sudochats) == 0:
        return await edit_delete(
            event, "**• يبدو انك لم تقوم بأضافة مستخدم متحكم لجمثون افاتار الخاص بك**"
        )
    result = "**قائمة المستخدمين المتحكمين بتنصيب جمثون افاتار :**\n\n"
